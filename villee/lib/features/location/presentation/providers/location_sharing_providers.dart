...
final locationSharingRepositoryProvider =
    Provider<LocationSharingRepository>((ref) {
...
class LocationSharingController extends AsyncNotifier<void> {
  ...
  Future<bool> enableSharing() async {
    ...
    state = await AsyncValue.guard(() async {
      ...
      if (accessState.status != LocationAccessStatus.ready ||
        ...
        throw LocationSharingUnavailableException(
          accessState.status,
        );
      }
  ...
  Future<void> _checkTimedUpdate() async {
    ...
    try {
      ...
      debugPrint(
        'Periodic location update failed: $error',
      );
    }
  }
  ...
  Future<void> _uploadIfNeeded(Position position) async {
    ...
    final elapsedEnough =
        _lastUploadedAt == null ||
        now.difference(_lastUploadedAt!) >=
            _locationUploadInterval;
